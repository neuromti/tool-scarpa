...
def test_stack_template_len(pcount, plen, explen):
    template = arange(0, plen)
    signal = stack_template(template, periodcount=pcount)
...
def test_stack_template_val(pcount, plen, expval):
...
def test_stack_template_raises(pcount):
